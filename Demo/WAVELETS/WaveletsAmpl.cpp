...
WaveletsAmpl::WaveletsAmpl(int n, int m, float lx, float lz, float a, float v,
...
int WaveletsAmpl::pos2idx(int n, int m) const
{
    return m * N + n;
}

complex<float> WaveletsAmpl::getK(int n, int m) const
{
    return { 2.f * pi * (static_cast<float>(n) - static_cast<float>(N) / 2.f) / Lx,
            2.f * pi * (static_cast<float>(m) - static_cast<float>(M) / 2.f) / Lz };
}

float WaveletsAmpl::Ph(complex<float> k) const
{
    float k_abs = abs(k);
    if (k_abs < EPSILON)
        return 0.0f;
...
std::complex<float> WaveletsAmpl::hTlide0(complex<float> k)
{
...
std::complex<float> WaveletsAmpl::hTlide(int n, int m, float t) const
{
